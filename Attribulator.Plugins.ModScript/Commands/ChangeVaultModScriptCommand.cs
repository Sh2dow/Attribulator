...
namespace Attribulator.Plugins.ModScript.Commands
{
	...
	public class ChangeVaultModScriptCommand : BaseModScriptCommand
	{
		...
		public override void Execute(DatabaseHelper databaseHelper)
		{
			...
			databaseHelper.Database.Vaults.TryGetValue(VaultName, out var vault);
			...
			if (vault == null)
			{
				if (collection.Class.Name == "gameplay")
				{
					...
					databaseHelper.Database.Vaults.Add(vault.Name, vault);
				}
				else
				{
			...
			if (!oldVaultCollections.Any())
			{
				...
				databaseHelper.Database.Vaults.Remove(oldVault.Name);
			}
